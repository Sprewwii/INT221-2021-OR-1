    ...
    <div class="relative inline-block">
      <div class="flex space-x-2">
        <button
          ...
          class="relative z-10 w-30 sm:w-48 flex items-center justify-between py-2 sm:py-3 px-1 sm:pl-2 text-sm text-center text-manapurple-100 outline outline-1 outline-manapurple-100 border border-transparent rounded-md hover:bg-manapurple-100 hover:text-white"
        >
          <span
            v-if="selectedFilter.id === 4"
            class="mx-1 w-20 sm:w-48 truncate"
            >{{ filterList[3].selectedCategory.categoryName }}</span
          >
          <span
            v-else-if="selectedFilter.id === 5"
            class="mx-1 w-20 sm:w-48 truncate"
            >{{
              new Date(choosedDate).toLocaleString("en-US", {
                year: "numeric",
                month: "short",
                day: "numeric",
              })
          ...
          >
          <span v-else class="mx-1 w-20 sm:w-48 truncate">{{
            selectedFilter.name
          }}</span>
          <svg
            class="w-5 h-5 mx-1"
      ...
      >
        <div
          v-show="showFilterTab === 4"
          class="w-40 sm:w-64 background-gray h-80 z-20"
          @mouseover="showFilterTab = 4"
          @mouseleave="showFilterTab = 0"
        >
          <div
            v-for="eventCategory in eventCategories"
            :key="eventCategory.categoryId"
            @click="
              filterCategory(eventCategory.categoryId);
              filterList[3].selectedCategory = eventCategory;
            "
            :class="[
              filterList[3].selectedCategory &&
              filterList[3].selectedCategory.categoryId ===
                eventCategory.categoryId
                ? 'bg-manapink-100 text-white'
                : 'text-managray-100',
            ]"
        ...
        <div
          v-show="showFilterTab === 5"
          class="w-44 sm:w-64 h-80 z-20"
          @mouseover="showFilterTab = 5"
          @mouseleave="showFilterTab = 0"
        >
          <Datepicker
            v-model="choosedDate"
            @update:modelValue="filterDate"
            inline
            autoApply
        ...
        <div class="w-40">
          <div
            ...
            :key="index"
            @click="
              chooseFilter(filterList[index - 1]);
              $emit('noEventMessage', filterList[index - 1].noEventMessage);
            "
          ...
          <div
            ...
            :key="index"
            @mouseover="
              showFilterTab = filterList[index + 2].id;
              $emit('noEventMessage', filterList[index + 2].noEventMessage);
            "
            @mouseleave="showFilterTab = 0"
            class="block px-4 py-3 text-sm text-gray-400 capitalize transition-colors duration-200 transform hover:bg-manapurple-100 hover:text-white"
          >
