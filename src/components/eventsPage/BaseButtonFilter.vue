    ...
    <div class="relative inline-block">
      <div class="flex space-x-2">
        ...
        <button @click="toggleFilterList()"
          ...
          <span v-if="selectedFilter.id === 4" class="mx-1 w-20 sm:w-48 truncate">{{
              filterList[3].selectedCategory.categoryName
          }}</span>
          <span v-else-if="selectedFilter.id === 5" class="mx-1 w-20 sm:w-48 truncate">{{ new
              Date(choosedDate).toLocaleString("en-US", {
                year: "numeric",
                month: "short",
                day: "numeric",
              })
          }}</span>
          <span v-else class="mx-1 w-20 sm:w-48 truncate">{{ selectedFilter.name }}</span>
      ...
      <div v-show="showFilterList"
        ...
        <div v-show="showFilterTab === 5" class="w-44 sm:w-64 h-80 z-20" @mouseover="showFilterTab = 5"
          @mouseleave="showFilterTab = 0">
          <Datepicker v-model="choosedDate" @update:modelValue="filterDate" inline autoApply :enableTimePicker="false"
            dark />
        </div>
        <div class="w-40">
          ...
          <div v-for="index in 2" :key="index"
            @mouseover="showFilterTab = filterList[index + 2].id; $emit('noEventMessage', filterList[index + 2].noEventMessage)"
            @mouseleave="showFilterTab = 0"
            class="block px-4 py-3 text-sm text-gray-400 capitalize transition-colors duration-200 transform hover:bg-manapurple-100 hover:text-white ">
